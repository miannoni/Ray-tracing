...
class drop : public hittable {
    public:
        drop() {}
        drop(point3 cen, double size_x, double size_y, double size_z, shared_ptr<material> m) : center(cen), size_x(size_x), size_y(size_y), size_z(size_z), mat_ptr(m) {};
...
bool drop::hit(const ray& r, double t_min, double t_max, hit_record& rec) const {
    ...
    // double v = acos ((rec.p - center).z()/(rec.p - center).length());
	// double u = asin (((rec.p - center).y()/(rec.p - center).length())/sin(v));

    //original xyz
	// double u = acos ((rec.p - center).z()/(rec.p - center).length());
	// double v = 2*((rec.p - center).y()/(rec.p - center).length());
	// v = v/((1 - cos(u))*sin(u));
	// v = asin(v);

	// flipped xyz
	double u = acos (-(rec.p - center).x()/(rec.p - center).length());
	double v = 2.0*((rec.p - center).z()/(rec.p - center).length());
	v = v/((1.0 - cos(u))*sin(u));
	v = acos(v);

	double dxdu = cos(v) * (0.5 * (sin(u) * sin(u)) - (0.5 * (cos(u) - 1.0)) * cos(u));
	double dxdv = 0.5 * sin(u) * (cos(u) - 1.0) * sin(v);

	double dydu = sin(v) * ((0.5 * (sin(u) * sin(u))) - (0.5 * (cos(u) - 1.0) * cos(u)));
	double dydv = -0.5 * sin(u) * (cos(u) - 1.0) * cos(v);

	double dzdu = -sin(u);
	double dzdv = 0;


	// changed for xyz swap 
	// double ez = oc.x()*size_x;
    // double ex = -oc.y()*size_y;
    // double ey = oc.z()*size_z;
	// vec3 tu(dydu, dzdu, dxdu);
    // vec3 tv(dydv, dzdv, dxdv);

	// original
	vec3 tu(dxdu,dydu,dzdu);
    vec3 tv(dxdv,dydv,dzdv);
	
    vec3 outward_normal = cross(tv, tu);
